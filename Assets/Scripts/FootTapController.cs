...
public class FootTapController : MonoBehaviour
{
    ...
    public GameObject background, FootDotPrefab, linePrefab;
    ...
    void CheckFootisUp(ref bool isUp, ref bool isTapping, int newTap,ref GameObject img, int threshold=550) {
        if (DataReceiver.footDataList[newTap] > threshold) {
            isUp = true;
            img.SetActive(false);
            if (main.Mode == Main.DataReceiveMode.mobile) { // refactor later
                DataReceiver.footDataList[newTap] = 0;
            }
        } else {
            isUp = false;
            img.SetActive(true);
        ...
        if (!isUp && isTapping) { // then this moment is tapping (foot down)
            ...
            // draw line
            // if (last > 0 && hasArrowIcon[last, curr] == 0) {
            //     DrawLine(dotCoordinates[last], dotCoordinates[curr]);
            //     hasArrowIcon[last, curr]++;
            // }
    ...
    void DrawWhiteDot(int curr) {
       ...
       GameObject dotObject = Instantiate<GameObject>(FootDotPrefab, FootDotContainer);
        ...
        Text dotText = dotObject.GetComponentInChildren<Text>();
        dotText.text = TapNumber.ToString();       
    }
